...
pub fn check_bullet_tank_collisions(
    ...
    for (bullet_idx, bullet) in bullets.iter().enumerate() {
        ...
        if !bullet.from_player
            && bullet.collides_with_circle(&player_tank.position, player_tank.size)
        {
            if !player_tank.is_invincible() {
                let damage = (bullet.damage as f32 * player_tank.get_damage_multiplier()) as i32;
                if player_tank.take_damage(damage) {
                    // 玩家死亡将在游戏主循环中处理
                }
            }
        ...
        for (tank_idx, tank) in tanks.iter_mut().enumerate() {
            if bullet.from_player && bullet.collides_with_circle(&tank.position, tank.size) {
                if !tank.is_invincible() {
                    let damage =
                        (bullet.damage as f32 * player_tank.get_damage_multiplier()) as i32;
